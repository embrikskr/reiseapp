...
export default function PostCard({ post }: { post: Post }) {
  const cover = post.post_images?.[0]?.url || null; const countryName = nameFor(post.country_code, 'nb')
  return (<article className="card overflow-hidden">
    {cover && (<div className="relative w-full aspect-[16/10] bg-slate-100">
      <Image src={cover} alt={post.title || countryName} fill priority={false} sizes="(max-width: 768px) 100vw, 800px" style={{objectFit:'cover'}} />
      <div className="absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-black/40 to-transparent" />
    </div>)}
    <div className="p-4">
      <div className="flex items-center justify-between">
        <a href={`/profile/${post.profiles.username}`} className="font-semibold hover:underline">@{post.profiles.username}</a>
      ...
      <div className="mt-2 flex items-center gap-2"><span className="badge">{countryName}</span><RatingStars value={post.rating} readOnly /></div>
